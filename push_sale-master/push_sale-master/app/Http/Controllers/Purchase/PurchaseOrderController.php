...
class PurchaseOrderController extends Controller
{
    public function index(Request $request)
    {
        $user = Auth::user();
        if ($user) {
            $data = PurchaseOrder::where("type", "invoice_in")->with("orderitem")->select(["id","code","purchase_date","total_amount"])->orderBy('purchase_date', 'desc')->get();
    ...
    public function setOrderShipped(Request $request)
    {
        ...
        if ($user) {
            ...
            try {
                $data = $request->all();
                DB::beginTransaction();
                $po = PurchaseOrder::where("id", $data["id"])->where("state", "in_way")->first();
                ...
                if (isset($data["delivery_proof"])) {
                    $path = storage_path('app/public') . "/delivery_proof" . "/" . $data["id"] . ".jpg";
    ...
    public function setNewCashOrder(Request $request)
    {
        ...
        if ($user) {
            ...
            try {
                $actor = Actor::where("user_id", $user->id)->first();
                DB::beginTransaction();
                $po = PurchaseOrder::where("id", $data["purchaseorder_id"])->first();
                if ($po) {
            ...
            try {
                ...
                foreach ($data as $item) {
                    ...
                    if ($item["cashed"] > 0) {
                        ...
                        //$item["cashed"];
                        $po = PurchaseOrder::where("id", $item["purchaseorder_id"])->where("state", "shipped")->first();
                        if ($po) {
    ...
    public function changePlannedDate(Request $request)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                PurchaseOrder::where("order_id", $data["order_id"])->update([
                    "planned_delivery_date" => $data["date"]
                ]);
    ...
    public function renewOrder(Request $request){
        ...
        if ($user) {
            ...
            $data = $request->all();
            DB::beginTransaction();
            $order = PurchaseOrder::where("order_id",$data["order_id"])->where("state","expired")->orderBy("created_at","desc")->first();
