        ...
        if ($user) {
            ...
            try {
				...
				if ($request->hasFile('file')) {
					...
					$img = "/products/" . $product->id . "/" . uniqid("pro_") . $product->id . "." . $file->getClientOriginalName();
					$filePath = storage_path('app/public') . $img;
					if(!file_exists(storage_path('app/public') . "/products/" . $product->id)){
						mkdir(storage_path('app/public') . "/products/" . $product->id);
					}
					// $path = $file->storeAs("products/{$product->id}", "pro_{$product->id}.{$file->getClientOriginalName()}", 'public');
					Product::where("id", $product->id)->update(["image" => "/storage/{$img}"]);
