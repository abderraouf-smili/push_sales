    ...
    public function adjustement(Request $request)
    {
        try {
            ...
            if ($user) {
                ...
                $outOfStock = StockQuantity::checkForAdjust($warehouse->id,$data["stock"]);
                PriceListItem::adjuster($data["prices"], $actor->distributor_id);
                if(count($outOfStock)>0){
                    //stock has some out of stock
                    DB::commit();
                    return response()->json(["status"=>"FAIL","message"=>"out of stock","data"=>$outOfStock]);
                }else{
                    StockQuantity::adjuster($warehouse->id,$data["stock"]);
                }
                DB::commit();
