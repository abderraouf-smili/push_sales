    ...
    public function check(Request $request)
    {
        try {
            ...
            if ($user) {
                ...
                $amount_distributor = [];
                $warehouse_ids = [];
                $warehouses = TenantGuard::warehouses(array_keys($data["warehouses_amount"]), $actor);
                if ($warehouses->count() != count(array_unique(array_filter(array_keys($data["warehouses_amount"]))))) {
                    return TenantGuard::forbiddenResponse();
                }
